...
Пишите что вы думаете почему такие технологии как dconf игнорируют корпорации и все остальные и не пишут свои желтые статьи итд. Думаю некоторые поняли что dconf это такая вещь о которой просто не будут говорить в силу того что из него можно вытащить хорошии флаги и увезти в dbus и другие технологии я получил этот конфиг сложнеишими методами установкой и смешиванием различных конфигурации и этому конфигу уже пять с небольшим лет сам метод возможно потом поможет сделать нормальную операционную систему. Понятно что инвестиции за настройку не дают по причине зависти ведь что бы настроить так как в этом конфиге надо перефармить очень много систем что не любят быстро-около-денег-зарабатывальщики. То же самое происходит с ядром которое было собрано на моей системе версии oem 5.6.0 как бы игнорируют что оно очень удачное и может поддерживать arm и arm64 , но тем не менее берут и приспосабливают к своему продукту который выпускают как свои полный проект возможно отрезав некоторые модули для легкости я конечно одобряю только когда указано откуда взято ядро , а не уведено тихим кликом , само ядро может делить в эмуляторе 4 ядра на 8 как показано тут https://youtu.be/Cq1bE6Gu1Hw?t=316. Впрочем что касается остальных de у них пока не у всех есть dconf в частности тот же kde и всякие icewm это страдальцы которые будут кричать что dconf это regedit и нам с вами не попути тем не менее наяривая очередную игрушку через wine не видя что dconf это не regedit ни коим образом это андроид. Новое ядро я не собираю по причине что мне не донатят что бы собрать мощную конфигурацию оборудования что бы перефармливать те же ядра и искать экзешен лучший вариант лучше всего и быстрее на 64 битном процессоре по скольку -j64 решит проблему времени и вернемся к ядру и заготовкам все себе забирают около кремлеботные так как я и писал они находятся около денег так мало того и не подпускают еще к заслуженным инвестициям или же донатам которые могут и без ихней помощи мне прийти из других стран в целом это обще мировая конкуренция накладывает отпечаток что в мире большая конкуренцая играет не только как конкуренция , а еще и как ширма для тех кто ближе к деньгам эмулируя честную конкуренцию с помощью иллюзии и где то уничтожая хороший код чем сами и роют себе и обще мировому прогрессу яму , далее об одной из ям. На сегодня я считаю это ядро 5.9.3/5.13.2 пока самым удавшимся и одновреммено не удачным из за отсутствия поддержки anbox , nvidia , dkms realtime и другого проперти https://github.com/Griggorii/linux-image-5.9.3_amd64.deb_light_zstd_ultra_fast_kernel_wayland_fast/releases/tag/fast ядро 5.9.3/5.13.2 не имеет бага с pulse-audio и вырываю одну из ям это блютус девайс устройства на этом ядре не надо будет постоянно переконнекить по скольку блютус аудио устройство будет работать с разными наушниками с первого раза подавая звук именно в нужный инпуит как это устроено в андроид , в других же ядрах возможно что бы звук появился придется делать реконнект как мы это любим два раза так что возможно и это влияет на производительность и где т о зовется багом 12309 связанную с зиканием звука и каким нибудь acm кодеком (про кодек это предположение по скольку в современном вайне явно сним что то не так) , если же не нужен anbox и другое пропиеритари то ядро 5.6.0-1020 , его фишка в том что оно на более новых gcc не собирается и некоторые модули мне пришлось подключать в конфигурационных фаилах самому , возможно на нем в режиме root будет поддерживаться nvidia 440.100 пакеты которых можно взять тут https://github.com/Griggorii/linux-image-oem-5.6.0-1020-kernel-ubuntu-x86_64-deb-package про это 5.6.0 могу предьявить доказательства что что то не ладно с яндекс сайтом по скольку ядро обращалось на хост-сайт провайдера яндекс примерно известно кто возглавляет совет директоров итд драйвера nvidia 440.100 именно в яндекс ядро сохранено в виде дампа срс бинарного кода где нужна лишь до сборка dkms весь сжатый сбилденный мною пак весит 5 Гигабайт и в распаковке окого 20 Гигабайт так как я понял что это огромная работа я сделал дамп полностью все и могу продать за 10 миллионов рублей по скольку в истории такое ядро 1 на миллион раз даже в заговор вошел яндекс остается его отключить от яндекса и направить на драивер версии выше и на лету получить это же ядро или уменьшить его в весе отключив кучу модулей и собрав за 15 секунд
...
Спасти линукс можно тут http://chng.it/WDfJZLPgZZ причина очень проста некоторые думают что это за мусор на гит хаб и все такое как найти среди горы мусора линк на iso образ , а объяснение просто это не мусор это доказательство именно настоящей работы которую нельзя выбрасывать по причине первичности ибо потом не докажешь что разработан был данный linux дистрибутив именно всякими такими методами потому что один вид метода не подходит по причине что одним только подходом не разработать объектно ориентированный код позже над которым проведут обвязку и признают его онли уникальным и заменой старому легаси по одной простой причине он будет в 10 раз быстрее ведь если у меня тут найти на гитхабе chromium os 77 и начать соревноваться в быстрой загрзке с любой из ныне существующих операционной систем то ни одна система не загрузиться с такой скоростью и проиграет по скольку этот chromium os 77 унаследовал некие параметры с ранее разрабатываемой мною ubuntu 16.04 , да мне самому не пристало держать кучу этого всего если бы корпорации мне не задушили инвестиции , а наоборот сосредоточились выявить точечными подходами что я действительно внес изменения в код и теперь meson может его дампнуть одним движением руки и выдать в мир новую операционную систему на основе этой разработки уже освобожденную от багов и тормозов ведь все придет к тому что я ранее и рассказывал очень быстрому qt4 или же авторов типа нового фраимворка заставят скопилефтить сгенерированные параметры в более новые версии и более точными цифрами что бы тормоза исчезли и версия прибавила циферку да это будет типа qt6 и выше тогда ux может и перстанет тормозит потому что будет наследование ускорения. Так же разработан план по ускорению разработки где я привожу факты 1) https://otvet.mail.ru/question/224888589 2) https://otvet.mail.ru/question/224903774 есть еще один интересный скрипт я не технологический паразит по этому напишу и оставлю его как есть он удаляет все линки в определенной дериктории и может пригодиться лучше взять отсюда без всяких форматировании https://raw.githubusercontent.com/Griggorii/Setting_dconf_linux_OS20.04_V46.0_By_Griggorii_Wayland_adaptation/main/README.md:
#!/bin/sh
#
